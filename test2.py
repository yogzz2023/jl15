...
def chi_square_gating(Z, kalman_filter):
    ...
    print(f"d2: {d2}, Gate Threshold: {gate_threshold}")
...
def form_clusters(measurements, kalman_filter):
    ...
    for measurement in measurements:
        ...
        if chi_square_gating(Z, kalman_filter):
...
def jpda(clusters, kalman_filter):
    ...
    return best_hypothesis, marginal_probabilities[best_hypothesis_index]
...
if __name__ == "__main__":
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        for i, (r, az, el, mt) in enumerate(group):
            print(f"Measurement {i + 1}: (r={r}, az={az}, el={el}, t={mt})")
            ...
            if not kalman_filter.first_rep_flag:
                ...
                hypothesis= generate_hypotheses(clusters)
                print(f"Number of hypothesis formed: {len(hypothesis)}")
                print(f"hypothesis formed: {hypothesis}")



                if clusters:
                    best_hypothesis, best_prob = jpda(clusters, kalman_filter)
    ...
    # plt.scatter(time_list, rng, label='filtered range (code)', color='green', marker='*')
    ...
    # plt.figure(figsize=(12, 6))
    # plt.subplot(facecolor="white")
    # plt.scatter(time_list, azm, label='filtered azimuth (code)', color='green', marker='*')
    ...
    # plt.figure(figsize=(12, 6))
    # plt.subplot(facecolor="white")
    # plt.scatter(time_list, ele, label='filtered elevation (code)', color='green', marker='*')
